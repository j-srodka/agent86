...
/**
 * Static linkage: both reference adapters are part of the MCP server binary for mixed routing.
 */
void [V0_ADAPTER_FINGERPRINT.name, PY_ADAPTER_FINGERPRINT.name];

export type SupportedLanguage = "ts" | "py";
...
export function assertSupportedLanguage(filePath: string): SupportedLanguage {
  ...
  if (lang === null) {
    ...
    throw new McpError(
      ...
      `Unsupported source extension ${ext}; supported: .ts, .py`,
      {
        file_path: filePath,
        extension: ext,
        supported_extensions: [".ts", ".py"],
      },
    );
  }
  return lang;
