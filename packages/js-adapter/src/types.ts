/**
 * Wire types for js-adapter — structurally compatible with ts/py adapters but using
 * JavaScript-specific unit kind strings. Agents branch on `outcome` and `entries[].code`.
 */
...
/** JavaScript unit kinds (v1). See docs/impl/v0-decisions.md — JavaScript adapter (js-adapter, v1). */
export type JsUnitKind =
...
/** Source file omitted because Tree-sitter threw during parse. */
export interface SkippedJsParseThrow {
  file_path: string;
  reason: "parse_throw";
}
...
export interface WorkspaceSnapshot {
  ...
  /** Unused on js-adapter wire (always `[]`); kept for interchange shape parity with ts/py. */
  skipped_tsx_paths: string[];
  /** `.jsx` files discovered but not parsed with the plain JS grammar (v1). */
