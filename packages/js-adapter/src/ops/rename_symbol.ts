...
import type { JsUnitKind, LogicalUnit, RenameSurfaceReport, WorkspaceSnapshot } from "../types.js";
import { declaredNameFromUnitSource } from "../units.js";

/** Cross-file `rename_surface_report.found` threshold for `lang.js.cross_file_rename_broad_match`. */
...
function renameInSource(
  ...
  const tree = parseJavaScriptSource(canonical);
...
export interface RenameSymbolErr {
  ok: false;
  message: string;
  code?: "lang.js.rename_unsupported_node_kind" | "parse_error";
}

export type RenameSymbolResult = RenameSymbolOk | RenameSymbolErr;

/**
 * `rename_symbol` for JavaScript: identifier-level rename within the declaring file,
 * optional best-effort cross-file identifier rewrite (no type checker).
 */
export async function applyRenameSymbol(input: RenameSymbolInput): Promise<RenameSymbolResult> {
  ...
  if (!RENAMEABLE_KINDS.has(unit.kind)) {
    return {
      ok: false,
      message: "rename_symbol supports function_declaration, class_declaration, method_definition, arrow_function only",
      code: "lang.js.rename_unsupported_node_kind",
    };
  }
  ...
  const oldName = declaredNameFromUnitSource(span);
  if (!oldName) {
  ...
  for (const [, text] of outMap) {
    const t = parseJavaScriptSource(text);
