...
import { declaredNameFromUnitSource, extractJsLogicalUnits } from "../units.js";
...
function isJsDestinationPath(posix: string): boolean {
...
export async function applyMoveUnit(input: MoveUnitInput): Promise<MoveUnitResult> {
  ...
  if (!isJsDestinationPath(destinationFilePosix)) {
    return {
      ok: false,
      code: "parse_error",
      message: "move_unit: destination_file must be a repo-relative .js, .mjs, or .cjs path",
    };
  }
  ...
  if (sourcePath === destinationFilePosix) {
    return {
      ok: false,
      code: "lang.js.move_unit_same_file",
      message:
        "[lang.js.move_unit_same_file] destination_file equals source file; same-file reorder is out of scope for v1",
    };
  }
  ...
  if (destCanonical.length > 0) {
    const tree = parseJavaScriptSource(destCanonical);
    const spans = extractJsLogicalUnits(tree, {
      grammarDigest: JS_GRAMMAR_DIGEST,
    ...
    for (const span of spans) {
      ...
      if (n === movedName) {
        return {
          ok: false,
          code: "lang.js.move_unit_name_conflict",
          message: `[lang.js.move_unit_name_conflict] destination file already declares a unit named "${movedName}"`,
        };
      }
    }
  }
