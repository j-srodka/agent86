...
export const PY_ADAPTER_FINGERPRINT: AdapterFingerprint = {
  name: "py-adapter",
  semver: "0.0.0",
...
function computeSnapshotId(input: {
  ...
  const payload = {
    ...
    files: sortedFiles,
    units: sortedUnits,
    // Keep field name consistent with ts-adapter wire shape for interchange tooling.
...
export async function materializeSnapshot(options: MaterializeSnapshotOptions): Promise<WorkspaceSnapshot> {
  ...
  const grammarDigest = PY_GRAMMAR_DIGEST;
  const previousSnapshot = options.previousSnapshot;

  const pyRel: string[] = [];
  await walkPyFiles(snapshotRootResolved, snapshotRootResolved, pyRel);
  pyRel.sort((a, b) => a.localeCompare(b));

  const files: SnapshotFile[] = [];
  const units: LogicalUnit[] = [];
  const skipped_parse_throw: SkippedPyParseThrow[] = [];
  ...
  for (const rel of pyRel) {
    ...
    let tree: ReturnType<typeof parsePythonSource>;
    try {
      tree = parsePythonSource(canonical);
    } catch {
      skipped_parse_throw.push({ file_path: rel, reason: "parse_throw" });
      continue;
    }
    const fileSha = sha256HexOfCanonicalSource(canonical);
    // Python files: all authored by default (no generated-file detection for v1).
