...
import { assertPyGrammarDigestPinned, PY_GRAMMAR_DIGEST } from "./grammar.js";
...
function applyingAdapterFingerprint(): AdapterFingerprint {
  return { ...PY_ADAPTER_FINGERPRINT };
}

/**
 * Atomic v0 apply for Python: validate §9 gates, then run ops sequentially;
...
export async function applyBatch(input: ApplyBatchInput): Promise<ValidationReport> {
  ...
  if (snapshot.grammar_digest !== PY_GRAMMAR_DIGEST) {
    return buildFailureReport({
      snapshot_id: snapshot.snapshot_id,
      adapter,
      toolchain_fingerprint_at_apply: toolchainFingerprintAtApply,
      entries: [
        entry(
          "grammar_mismatch",
          `[gate:snapshot_grammar_digest] WorkspaceSnapshot.grammar_digest (${snapshot.grammar_digest}) does not match applying adapter (${PY_GRAMMAR_DIGEST}); snapshot may be stale or produced by a different toolchain.`,
          null,
          null,
        ),
      ],
  ...
  const expected = PY_ADAPTER_FINGERPRINT;
  if (
    ...
    return buildFailureReport({
      ...
      entries: [
        entry(
          ...
          "AdapterFingerprint on snapshot does not match applying py-adapter build (v0); use a snapshot from this adapter or upgrade.",
          null,
          null,
        ),
      ],
      ...
      if (op.op === "rename_symbol") {
        ...
        if (!r.ok) {
          ...
          const code: ValidationEntry["code"] =
            r.code === "lang.py.rename_unsupported_node_kind"
              ? "lang.py.rename_unsupported_node_kind"
              : "parse_error";
          return buildFailureReport({
            snapshot_id: snapshot.snapshot_id,
            adapter,
        ...
        if (op.cross_file && rs.found > CROSS_FILE_RENAME_BROAD_MATCH_THRESHOLD) {
          renameSurfaceEntries.push({
            code: "lang.py.cross_file_rename_broad_match",
            severity: "warning",
