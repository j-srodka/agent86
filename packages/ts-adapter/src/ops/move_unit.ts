 ...
 *
 * Import/export rewriting in other files is intentionally NOT performed — callers must update imports;
 * see `docs/impl/v0-decisions.md` and comments on `applyBatch` in `apply.ts`.
 */
export async function applyMoveUnit(input: MoveUnitInput): Promise<MoveUnitResult> {
  ...
  if (!destinationFilePosix.endsWith(".ts") || destinationFilePosix.endsWith(".tsx")) {
    return {
      ok: false,
      code: "parse_error",
      message: "move_unit: destination_file must be a repo-relative .ts path (not .tsx)",
    };
  }
  ...
  if (sourcePath === destinationFilePosix) {
    return {
      ok: false,
      code: "lang.ts.move_unit_same_file",
      message:
        "[lang.ts.move_unit_same_file] destination_file equals source file; same-file reorder is out of scope for v1",
    };
  }
  ...
  if (destCanonical.length > 0) {
    const tree = parseTypeScriptSource(destCanonical);
    const spans = extractLogicalUnits(tree, {
      grammarDigest: snapshot.grammar_digest,
    ...
    for (const span of spans) {
      ...
      if (n === movedName) {
        return {
          ok: false,
          code: "lang.ts.move_unit_name_conflict",
          message: `[lang.ts.move_unit_name_conflict] destination file already declares a unit named "${movedName}"`,
        };
      }
    }
  }
